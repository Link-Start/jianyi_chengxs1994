...
手机端采用预览、时间线和底部面板布局，底部可切换「素材」「属性」或收起面板。主视频轨道优先显示，工具栏可横向滑动；横屏时预览显示在左侧。当前已适配布局，触摸手势及手机浏览器导出兼容性仍待进一步验证。

## 基础功能

- 撤销 / 重做：保留最近 50 步编辑，一次拖动算一步，连续输入合并记录。

- 导入多段视频，拼接、分割、裁剪和删除片段；拖动主视频排序，拖动左右边缘裁剪。
- 时间线吸附：移动或裁剪时对齐播放头与片段边界，工具栏可开关，按住 `Alt` 临时关闭。
...
主视频拖动时显示插入位置，松开后连续排列；拖动两端裁剪时显示时长，`Esc` 可取消本次主视频拖动。裁剪受当前片段的分割边界限制。文字、贴纸和音频保持原时间位置；作品缩短时，超出总时长的部分会截短或移除。

时间线工具栏提供「撤销」「重做」。快捷键为 `⌘/Ctrl+Z`、`⌘/Ctrl+Shift+Z`，Windows 也支持 `Ctrl+Y`；输入框内保留原生文字撤销。支持恢复拖动排序、边缘裁剪、分割、删除、素材导入、文字、位置与属性修改，播放头移动、选中和缩放不单独记录。切换草稿或刷新页面会清空历史，历史不写入草稿文件；撤销后的内容仍参与自动保存。恢复素材时可能短暂等待解码，期间不能继续编辑。

打开页面直接进入编辑工作台，不自动弹出草稿列表。需要新建或打开已有草稿时，点击「素材」导入栏旁的「草稿」按钮。直接导入素材，会在首次自动保存时创建“未命名作品”。

列表中的每份草稿均显示完整 ID，可点击「复制 ID」用于 Skill 或 CLI 的 inspect / update / open 命令；若浏览器不允许自动复制，可在 ID 框内选中后手动复制。
...
js/history.js          有限撤销重做历史、操作合并和快捷键
js/timeline-edit.js    主视频拖放排序、边缘裁剪和统一吸附
