...
时间线工具栏提供「撤销」「重做」。快捷键为 `⌘/Ctrl+Z`、`⌘/Ctrl+Shift+Z`，Windows 也支持 `Ctrl+Y`；输入框内保留原生文字撤销。支持恢复分割、删除、素材导入、文字、位置与属性修改，播放头移动、选中和缩放不单独记录。切换草稿或刷新页面会清空历史，历史不写入草稿文件；撤销后的内容仍参与自动保存。恢复素材时可能短暂等待解码，期间不能继续编辑。
