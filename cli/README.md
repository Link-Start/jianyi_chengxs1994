...
**保持终端运行，结束前确认网页显示「已保存」，再按 Ctrl+C 停止。** 服务使用随机端口，仅监听本机 127.0.0.1，通过临时会话令牌连接；不会上传到外网。`--no-browser` 可配合 `open` 或 `create --open`，只输出编辑地址、不启动浏览器。`--json` 输出中的 `url` 可供工具打开；服务命令会持续运行，不应等待其退出才读取结果。
