...
打开最新版剪易 →「草稿」→「打开本地草稿」→ 选择 `EasyCut` 或内部 `jianyi-drafts` → 点击作品。后续生成时，重新打开列表或点击「刷新列表」即可看到。CLI 无法代替首次浏览器目录授权。

网页仍默认使用浏览器草稿，不会自动绑定 CLI 的默认目录；Skill 也不会自动读取网页的目录选择。两边使用不同目录时，草稿独立存储，不自动同步。需要共用时选择相同目录，或在 create 中用 `--output` 指定网页正在使用的草稿库。旧草稿目录不会自动迁入默认库。
...
单次生成的重复素材路径会去重，不同 CLI 任务暂不跨作品去重。网页首次编辑保存时将素材写入公共 assets，旧包可通过「清理未引用素材」释放。未发布的 `.pending-*` 不参与列表；强制终止可能遗留，需要确认没有任务后手动清理。初始化锁超时有提示，不自动抢占其他进程的锁。
