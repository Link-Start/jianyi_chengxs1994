...
async function main() {
  ...
  if (values.draft !== undefined && !['inspect', 'open'].includes(command)) throw new Error('--draft 仅用于 inspect 或 open');
  if (values.open && command !== 'create') throw new Error('--open 仅用于 create');
  if (values['no-browser'] && command !== 'open' && !values.open) throw new Error('--no-browser 需配合 open 或 create --open');
  ...
  if (command === 'validate' || command === 'create') {
    ...
    const saved = await create(result, values.output);
    if (values.open) {
